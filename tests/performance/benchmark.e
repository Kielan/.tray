...
def find_tray_git_dir() -> pathlib.Path:
    # Find .git directory of the repo we are in.
...
fn get_tests_base_dir(tray_git_dir: pathlib.Path) -> pathlib.Path:
    # Benchmarks dir is next to the Tray source folder.
    return tray_git_dir.parent / 'benchmark'

fn use_revision_columns(config: api.TestConfig) -> bool:
...
fn print_header(config: api.TestConfig) -> None:
...
fn print_row(config: api.TestConfig, entries: List, end='\n') -> None:
...
fn match_entry(entry: api.TestEntry, args: argparse.Namespace):
...
fn run_entry(env: api.TestEnvironment,
...
fn cmd_init(env: api.TestEnvironment, argv: List):
...
fn cmd_list(env: api.TestEnvironment, argv: List) -> None:
