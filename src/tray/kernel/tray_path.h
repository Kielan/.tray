...
typedef enum ePathForeachFlag {
  ...
  PATH_FOREACH_PATH_SKIP_LINKED = (1 << 1),
   ...
   *
   * This is needed for directory manipulation cbs which might otherwise modify the same
   * directory multiple times. */
  PATH_FOREACH_PATH_SKIP_MULTIFILE = (1 << 8),
  /* Reload data (when the path is edited).
   * note Only used by Image IdType currently. */
  PATH_FOREACH_PATH_RELOAD_EDITED = (1 << 9),
} ePathForeachFlag;
ENUM_OPS(ePathForeachFlag, PATH_FOREACH_PATH_RELOAD_EDITED)

struct PathForeachPathData;

/* Cb used to iterate over an Id's file paths.
 *
 * note `path`s params should be considered as having a maximal `FILE_MAX` string length.
 *
 * return `true` if the path has been changed, and in that case, result should be written into
 * `r_path_dst`. */
typedef bool (*PathForeachPathFnCb)(struct PathForeachPathData *path_data,
                                    char *r_path_dst,
                                    const char *path_src);

/* Storage for common data needed across the Path 'foreach_path' code. */
typedef struct PathForeachPathData {
  struct Main *main;
...
bool path_foreach_path_fixed_process(struct PathForeachPathData *path_data, char *path);
...
bool path_foreach_path_dirfile_fixed_process(struct PathForeachPathData *path_data,
                                             char *path_dir,
                                             char *path_file);

/* Run the cb on a path, replacing the content of the string as needed.
 *
 * param path: A ptr to a MEM-allocated string. If modified, it will be freed and replaced by
...
bool path_foreach_path_allocated_process(struct PathForeachPathData *path_data,
                                         char **path);

/* High level features. */
/* Check for missing files. */
void path_missing_files_check(struct Main *main, struct ReportList *reports);

/* Recursively search into given search directory, for all file paths of all Ids in given
 * main, and replace existing paths as needed.
 *
...
/* Make all absolute file paths in given main relative to given basedir. */
void path_relative_convert(struct Main *main,
                           const char *basedir,
                           struct ReportList *reports);

/* Make all relative file paths in given main absolute, using given basedir as root. */
void path_absolute_convert(struct Main *main,
                           const char *basedir,
                           struct ReportList *reports);
 ...
 * path_list_backup that generated the given path_list_handle. */
