...
bool idtype_cache_key_cmp(const void *key_a_v, const void *key_b_v)
{
...
void idtype_init(void)
{
  /* Initialize data-block types. */
  id_type_init();
}

const IdTypeInfo *idtype_get_info_from_idcode(const short id_code)
{
  int id_index = idtype_idcode_to_index(id_code);
...
const IdTypeInfo *idtype_get_info_from_id(const Id *id)
{
  return idtype_get_info_from_idcode(GS(id->name));
}
...
const char *idtype_idcode_to_name(const short idcode)
{
...
const char idtype_idcode_to_name_plural(const short idcode)
{
  const IdTypeInfo *id_type = idtype_get_info_from_idcode(idcode);
  lib_assert(id_type != NULL);
...
short idtype_idcode_from_name(const char *idtype_name)
{
...
bool idtype_idcode_is_valid(const short idcode)
{
  return idtype_get_info_from_idcode(idcode) != NULL ? true : false;
}

bool idtype_idcode_is_linkable(const short idcode)
{
  const IdTypeInfo *id_type = idtype_get_info_from_idcode(idcode);
  lib_assert(id_type != NULL);
...
bool idtype_idcode_is_only_appendable(const short idcode)
{
  const IdTypeInfo *id_type = idtype_get_info_from_idcode(idcode);
  lib_assert(id_type != NULL);
...
uint64_t idtype_idcode_to_idfilter(const short idcode)
{
...
short idtype_idcode_from_idfilter(const uint64_t idfilter)
{
#define CASE_IDFILTER(_id) \
  case FILTER_ID_##_id: \
    return ID_##_id
...
int idtype_idcode_to_index(const short idcode)
{
...
short idtype_idcode_from_index(const int index)
{
...
short idtype_idcode_iter_step(int *index)
{
  return (*index < ARRAY_SIZE(id_types)) ? idtype_idcode_from_index((*index)++) : 0;
}
                                 ...
                                 void *user_data)
{
  ...
  if (GS(id->name) == ID_SCE) {
    ...
    if (scene->master_collection != NULL) {
      type_info = idtype_get_info_from_id(&scene->master_collection->id);
      if (type_info->foreach_cache != NULL) {
