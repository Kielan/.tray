...
/* Allow using deprecated functionality for .tray file I/O. */
...
#include "tray_path.h"
#include "tray_colortools.h"
#include "tray_cxt.h"
#include "tray_cryptomatte.h"
#include "tray_global.h"
#include "tray_icons.h"
#include "tray_idprop.h"
#include "tray_idtype.h"
#include "tray_image_format.h"
#include "tray_lib_id.h"
#include "tray_lib_query.h"
#include "tray_main.h"
#include "tray_node.h"
#include "tray_node_tree_update.h"
...
using tray::Array;
using tray::Map;
using tray::MutableSpan;
using tray::Set;
using tray::Span;
using tray::Stack;
using tray::StringRef;
using tray::Vector;
using tray::VectorSet;
using tray::nodes::FieldInferencingInterface;
using tray::nodes::InputSocketFieldType;
using tray::nodes::NodeDeclaration;
using tray::nodes::OutputFieldDependency;
using tray::nodes::OutputSocketFieldType;
using tray::nodes::SocketDeclaration;
using namespace tray::nodes::node_tree_ref_types;
...
static void ntree_copy_data(Main *UNUSED(main), Id *id_dst, const Id *id_src, const int flag)
{
  ...
  LIST_FOREACH (const Node *, src_node, &ntree_src->nodes) {
    ...
    Node *new_node = tray::node_copy_with_mapping(
  ...
  if (ntree_src->previews && (flag & LIB_ID_COPY_NO_PREVIEW) == 0) {
    ...
    NODE_INSTANCE_HASH_ITER (iter, ntree_src->previews) {
      NodeInstanceKey key = node_instance_hash_iter_get_key(&iter);
      NodePreview *preview = (NodePreview *)node_instance_hash_iter_get_value(&iter);
  ...
  if (flag & LIB_ID_COPY_NO_PREVIEW) {
    ...
    previewimg_id_copy(&ntree_dst->id, &ntree_src->id);
  }
}
...
static void ntree_free_data(Id *id)
{
  ...
  /* free preview hash */
  if (ntree->previews) {
    node_instance_hash_free(ntree->previews, (NodeInstanceValueFP)node_preview_free);
  }

  if (ntree->id.tag & LIB_TAG_LOCALIZED) {
    libblock_free_data(&ntree->id, true);
  }

  previewimg_free(&ntree->preview);
}

static void lib_foreach_node_socket(LibForeachIdData *data, NodeSocket *sock)
{
  LIB_FOREACHID_PROCESS_FN_CALL(
      data,
      IDP_foreach_prop(
  ...
  switch ((eNodeSocketDatatype)sock->type) {
    case SOCK_OBJECT: {
      ...
      LIB_FOREACHID_PROCESS_IDSUPER(data, default_value->value, IDWALK_CB_USER);
      break;
    }
    case SOCK_IMAGE: {
...
static void node_foreach_id(Id *id, LibForeachIdData *data)
{
  ...
  LIB_FOREACHID_PROCESS_IDSUPER(data, ntree->gpd, IDWALK_CB_USER);

  LIST_FOREACH (Node *, node, &ntree->nodes) {
    LIB_FOREACHID_PROCESS_ID(data, node->id, IDWALK_CB_USER);

    LIB_FOREACHID_PROCESS_FN_CALL(
        data,
        IDP_foreach_prop(node->prop,
                             IDP_TYPE_FILTER_ID,
                             lib_query_idpropsForeachIdLink_cb,
                             data));
    LIST_FOREACH (NodeSocket *, sock, &node->inputs) {
      LIB_FOREACHID_PROCESS_FN_CALL(data, lib_foreach_node_socket(data, sock));
    }
    ...
    case NTREE_SHADER: {
      LIST_FOREACH (Node *, node, &ntree->nodes) {
        if (node->type == SH_NODE_SCRIPT) {
          ...
          path_foreach_path_fixed_process(path_data, nss->filepath);
        }
        else if (node->type == SH_NODE_TEX_IES) {
          NodeShaderTexIES *ies = reinterpret_cast<NodeShaderTexIES *>(node->storage);
          path_foreach_path_fixed_process(path_data, ies->filepath);
        }
      }
      break;
    }
...
static void write_node_socket_default_value(Writer *writer, NodeSocket *sock)
{
  ...
  switch ((eNodeSocketDatatype)sock->type) {
    case SOCK_FLOAT:
      ...
      break;
    case SOCK_VECTOR:
      loader_write_struct(writer, NodeSocketValueVector, sock->default_value);
      break;
    case SOCK_RGBA:
      loader_write_struct(writer, NodeSocketValueRGBA, sock->default_value);
      break;
    case SOCK_BOOLEAN:
      loader_write_struct(writer, NodeSocketValueBoolean, sock->default_value);
      break;
    case SOCK_INT:
      loader_write_struct(writer, NodeSocketValueInt, sock->default_value);
      break;
    case SOCK_STRING:
      loader_write_struct(writer, NodeSocketValueString, sock->default_value);
      break;
    case SOCK_OBJECT:
      loader_write_struct(writer, NodeSocketValueObject, sock->default_value);
      break;
    case SOCK_IMAGE:
      loader_write_struct(writer, NodeSocketValueImage, sock->default_value);
      break;
    case SOCK_COLLECTION:
      loader_write_struct(writer, NodeSocketValueCollection, sock->default_value);
      break;
    case SOCK_TEXTURE:
      loader_write_struct(writer, NodeSocketValueTexture, sock->default_value);
      break;
    case SOCK_MATERIAL:
      loader_write_struct(writer, NodeSocketValueMaterial, sock->default_value);
      break;
    case SOCK_CUSTOM:
...
static void write_node_socket(Writer *writer, NodeSocket *sock)
{
  loader_write_struct(writer, NodeSocket, sock);

  if (sock->prop) {
    IDP_Write(writer, sock->prop);
  }

  write_node_socket_default_value(writer, sock);
}
static void write_node_socket_interface(Writer *writer, NodeSocket *sock)
{
  loader_write_struct(writer, NodeSocket, sock);
...
void ntreeWrite(Writer *writer,NodeTree *ntree)
{
  id_write(writer, &ntree->id);

  if (ntree->adt) {
    animdata_write(writer, ntree->adt);
  }
  ...
  LIST_FOREACH (Node *, node, &ntree->nodes) {
    ...
    if (node->storage) {
      if (ELEM(ntree->type, NTREE_SHADER, NTREE_GEOMETRY) &&
        ...
        curvemapping_write(writer, (const CurveMapping *)node->storage);
      }
    ...
    if (node->type == CMP_NODE_OUTPUT_FILE) {
      ...
      image_format_write(writer, &nimf->format);

      LIST_FOREACH (NodeSocket *, sock, &node->inputs) {
        NodeImageMultiFileSocket *sockdata = (NodeImageMultiFileSocket *)sock->storage;
        loader_write_struct(writer, NodeImageMultiFileSocket, sockdata);
        image_format_write(writer, &sockdata->format);
      }
    }
  ...
  previewimg_write(writer, ntree->preview);
}

static void ntree_write(Writer *writer, Id *id, const void *id_address)
{
  NodeTree *ntree = (NodeTree *)id;
  ...
  ntreeWrite(writer, ntree);
}

static void direct_link_node_socket(DataReader *reader, NodeSocket *sock)
{
  loader_read_data_address(reader, &sock->prop);
  IDP_DataRead(reader, &sock->prop);
...
void ntreeReadData(DataReader *reader, NodeTree *ntree)
{
  ...
  ntree_update_tag_missing_runtime_data(ntree);

  loader_read_data_address(reader, &ntree->adt);
  animdata_read_data(reader, ntree->adt);
  ...
  LIST_FOREACH (Node *, node, &ntree->nodes) {
    ...
    if (node->type == CMP_NODE_OUTPUT_FILE) {
      LIST_FOREACH (NodeSocket *, sock, &node->inputs) {
        NodeImageMultiFileSocket *sockdata = (NodeImageMultiFileSocket *)sock->storage;
        image_format_read_data(reader, &sockdata->format);
      }
    }
  }
  ...
  previewimg_read(reader, ntree->preview);

  /* type verification is in lib-link */
}

static void ntree_read_data(DataReader *reader, Id *id)
{
  NodeTree *ntree = (NodeTree *)id;
  ntreeReadData(reader, ntree);
}

static void lib_link_node_socket(LibReader *reader, Lib *lib, NodeSocket *sock)
{
  IDP_ReadLib(reader, sock->prop);
...
static void lib_link_node_sockets(LibReader *reader, Lib *lib, List *sockets)
{
...
void ntree_read_lib(struct LibReader *reader, struct NodeTree *ntree)
{
  ...
  LIST_FOREACH (Node *, node, &ntree->nodes) {
     ...
     * of lib blocks that implement this. */
    IDP_ReadLib(reader, node->prop);
  ...
  /* Set node->typeinfo ptrs. This is done in lib linking, after the
   * first versioning that can change types still without functions that
   * update the typeinfo ptrs. Versioning after lib linking needs
   * these top be valid. */
...
static void ntree_read_lib(LibReader *reader, Id *id)
{
  NodeTree *ntree = (NodeTree *)id;
  ntreeReadLib(reader, ntree);
}

static void expand_node_socket(Expander *expander, NodeSocket *sock)
{
  IDP_ReadExpand(expander, sock->prop);
...
static void expand_node_sockets(Expander *expander, List *sockets)
{
  LIST_FOREACH (NodeSocket *, sock, sockets) {
...
void ntree_ReadExpand(Expander *expander, NodeTree *ntree)
{
  if (ntree->gpd) {
  ...
  LIST_FOREACH (Node *, node, &ntree->nodes) {
    ...
    IDP_ReadExpand(expander, node->prop);
...
static void ntree_read_expand(Expander *expander, Id *id)
{
  NodeTree *ntree = (NodeTree *)id;
  ntree_read_expand(expander, ntree);
}
...
IdTypeInfo IdType_ID_NT = {
    ...
    /* write */ ntree_write,
    /* read_data */ ntree_read_data,
    /* read_lib */ ntree_read_lib,
    /* read_expand */ ntree_read_expand,
...
/* Set specific typeinfo ptrs in all node trees on register/unregister */
...
void nodeRegisterType(NodeType *nt)
{
  ...
  if (nt->declare && !nt->declaration_is_dynamic) {
    if (nt->fixed_declaration == nullptr) {
      ...
      tray::nodes::NodeDeclarationBuilder builder{*nt->fixed_declaration};
      nt->declare(builder);
    }
  }
...
GHashIter *nodeTypeGetIter()
{
  return lib_ghashIter_new(nodetypes_hash);
}
...
Node *nodeFindRootParent(Node *node)
{
  if (node->parent) {
...
Node *nodeAddNode(const struct Cxt *C, NodeTree *ntree, const char *idname)
{
  ...
  ntree_update_tag_node_new(ntree, node);
                              ...
                              Map<const NodeSocket *, NodeSocket *> &socket_map)
{
  ...
  *node_dst = node_src;
  ...
  if (dst_tree) {
    ntree_update_tag_node_new(dst_tree, node_dst);
  }
    ...
    NodeTree *ntree, Node *fromnode, NodeSocket *fromsock, Node *tonode, NodeSocket *tosock)
{
  ...
  if (ntree) {
    ntree_update_tag_link_added(ntree, link);
  }
...
void nodeRemLink(NodeTree *ntree, NodeLink *link)
{
  ...
  if (ntree) {
    ntree_update_tag_link_removed(ntree);
  }
}
    ...
    if (link->fromnode == node) {
      if (link->fromsock->link) {
        ...
        if (fromlink) {
          ...
          ntree_update_tag_link_changed(ntree);
        }
        else {
          if (link->tosock->flag & SOCK_MULTI_INPUT) {
            adjust_multi_input_indices_after_removed_link(
...
void nodeFromView(const Node *node, float x, float y, float *rx, float *ry)
{
  if (node->parent) {
...
{
  for (const Node *parent_iter = node; parent_iter; parent_iter = parent_iter->parent) {
...
NodeTree *ntreeAddTree(Main *main, const char *name, const char *idname)
{
  ...
  NodeTree *ntree = (NodeTree *)libblock_alloc(main, ID_NT, name, flag);
  if (is_embedded) {
...
NodeTree *ntreeCopyTree_ex(const NodeTree *ntree, Main *main, const bool do_id_user)
{
  ...
  NodeTree *ntree_copy = (NodeTree *)id_copy_ex(main, (Id *)ntree, nullptr, flag);
  return ntree_copy;
}
...
bool node_preview_used(const Node *node)
{
  /* XXX check for closed nodes? */
  return (node->typeinfo->flag & NODE_PREVIEW) != 0;
}

NodePreview *node_preview_verify(NodeInstanceHash *previews,
                                 NodeInstanceKey key,
                                 const int xsize,
                                 const int ysize,
                                 const bool create)
{
  NodePreview *preview = (NodePreview *)node_instance_hash_lookup(previews, key);
  if (!preview) {
    if (create) {
...
void node_preview_free(NodePreview *preview)
{
  if (preview->rect) {
...
{
  LIST_FOREACH (Node *, node, &ntree->nodes) {
    NodeInstanceKey key = node_instance_key(parent_key, ntree, node);
...
void node_preview_init_tree(NodeTree *ntree, int xsize, int ysize)
{
  if (!ntree) {
    return;
  }
...
void node_preview_clear(NodePreview *preview)
{
...
void node_preview_clear_tree(NodeTree *ntree)
{
  if (!ntree || !ntree->previews) {
    return;
  }
  ...
  NODE_INSTANCE_HASH_ITER (iter, ntree->previews) {
    NodePreview *preview = (NodePreview *)node_instance_hash_iter_get_value(&iter);
    node_preview_clear(preview);
  }
}

void node_preview_merge_tree(NodeTree *to_ntree, NodeTree *from_ntree, bool remove_old)
{
  if (remove_old || !to_ntree->previews) {
    /* free old previews */
    if (to_ntree->previews) {
      node_instance_hash_free(to_ntree->previews, (NodeInstanceValueFP)node_preview_free);
    }
    ...
    if (from_ntree->previews) {
      ...
      NODE_INSTANCE_HASH_ITER (iter, from_ntree->previews) {
        ...
        node_instance_hash_remove(
            to_ntree->previews, key, (NodeInstanceValueFP)node_preview_free);
        node_instance_hash_insert(to_ntree->previews, key, preview);
      }

      /* NOTE: null free function here,
       * because ptrs have already been moved over to to_ntree->previews! */
...
static void node_free_node(NodeTree *ntree, Node *node)
{
  ...
  if (node->prop) {
    /* Remember, no Id user refcount management here! */
    IDP_FreePropContent_ex(node->prop, false);
    mem_freen(node->prop);
  }
  ...
  if (ntree) {
    ntree_update_tag_node_removed(ntree);
  }
}
...
void nodeRemoveNode(Main *main, NodeTree *ntree, Node *node, bool do_id_user)
{
  ...
  if (do_id_user) {
    ...
    if (node->typeinfo->freefn_api) {
      ApiPtr ptr;
      api_ptr_create((Id *)ntree, &Apiode, node, &ptr);

      node->typeinfo->freefn_api(&ptr);
    }
...
void ntreeFreeEmbeddedTree(NodeTree *ntree)
{
  ntreeFreeTree(ntree);
  libblock_free_data(&ntree->id, true);
  libblock_free_data_py(&ntree->id);
}
...
NodeTree **ntree_ptr_from_id(Id *id)
{
  switch (GS(id->name)) {
    case ID_MA:
...
{
  NodeTree **nodetree = ntree_ptr_from_id(id);
...
/* NODE TREE INTERFACE */
static NodeSocket *make_socket_interface(NodeTree *ntree,
                                         eNodeSocketInOut in_out,
                                         const char *idname,
                                         const char *name)
{
  ...
  if (in_out == SOCK_IN) {
    ...
    lib_snprintf(sock->id, MAX_NAME, "Output_%d", own_index);
  }
...
NodeSocket *ntreeAddSocketInterface(NodeTree *ntree,
                                    eNodeSocketInOut in_out,
                                    const char *idname,
                                    const char *name)
{
  ...
  ntree_update_tag_interface(ntree);
  return iosock;
}
...
/* generates a valid api id from the node tree name */
                               ...
                               char *description)
{
  ...
  identifier[0] = '\0';
  lib_uniquename_cb(
      ntree_interface_unique_id_check, nullptr, base, '_', id, maxlen);
...
StructApi *ntreeInterfaceTypeGet(NodeTree *ntree, bool create)
{
  if (ntree->interface_type) {
    ...
    if (!STREQLEN(api_struct_id(sapi), base, sizeof(base))) {
      ...
      /* rename the api type */
      api_def_struct_free_ptrs(&api, sapi);
      api_def_struct_identifier(&API, sapi, id);
      api_def_struct_ui_text(sapi, name, description);
      api_def_struct_duplicate_ptrs(&API, sapi);
    }
  }
  else if (create) {
...
{
  if (ntree->interface_type) {
    api_struct_free(&TRAY_API, ntree->interface_type);
    ntree->interface_type = nullptr;
  }
}

void ntreeInterfaceTypeUpdate(NodeTree *ntree)
{
  /* it would be sufficient to just recreate all props
   * instead of re-registering the whole struct type,
   * but there is currently no good way to do this in the api fns.
...
/* find stuff */
Node *ntreeFindType(const NodeTree *ntree, int type)
{
  if (ntree) {
    LIST_FOREACH (Node *, node, &ntree->nodes) {
      if (node->type == type) {
...
bool ntreeHasTree(const NodeTree *ntree, const NodeTree *lookup)
{
  if (ntree == lookup) {
    return true;
  }
  LIST_FOREACH (Node *, node, &ntree->nodes) {
    if (ELEM(node->type, NODE_GROUP, NODE_CUSTOM_GROUP) && node->id) {
      if (ntreeHasTree((NodeTree *)node->id, lookup)) {
        return true;
      }
    }
  }
...
NodeLink *nodeFindLink(NodeTree *ntree, const NodeSocket *from, const NodeSocket *to)
{
  LIST_FOREACH (NodeLink *, link, &ntree->links) {
...
int nodeCountSocketLinks(const NodeTree *ntree, const NodeSocket *sock)
{
  int tot = 0;
  LIST_FOREACH (const NodeLink *, link, &ntree->links) {
    if (link->fromsock == sock || link->tosock == sock) {
...
Node *nodeGetActive(NodeTree *ntree)
{
  if (ntree == nullptr) {
    return nullptr;
  }
...
void nodeSetSelected(Node *node, bool select)
{
  if (select) {
...
void nodeSetActive(NodeTree *ntree, Node *node)
{
...
int nodeSocketIsHidden(const NodeSocket *sock)
{
  return ((sock->flag & (SOCK_HIDDEN | SOCK_UNAVAIL)) != 0);
}

void nodeSetSocketAvailability(NodeTree *ntree, NodeSocket *sock, bool is_available)
{
  const bool was_available = (sock->flag & SOCK_UNAVAIL) == 0;
  if (is_available != was_available) {
    dune_ntree_update_tag_socket_availability(ntree, sock);
  }
...
int nodeSocketLinkLimit(const NodeSocket *sock)
{
  NodeSocketType *stype = sock->typeinfo;
...
static void update_socket_declarations(List *sockets,
                                       Span<tray::nodes::SocketDeclarationPtr> declarations)
{
  int index;
  LIST_FOREACH_INDEX (NodeSocket *, socket, sockets, index) {
    const SocketDeclaration &socket_decl = *declarations[index];
    socket->declaration = &socket_decl;
  }
}

void nodeSocketDeclarationsUpdate(Node *node)
{
  lib_assert(node->declaration != nullptr);
...
bool nodeDeclarationEnsureOnOutdatedNode(NodeTree *UNUSED(ntree), Node *node)
{
  ...
  if (node->typeinfo->declaration_is_dynamic) {
    node->declaration = new tray::nodes::NodeDeclaration();
    tray::nodes::NodeDeclarationBuilder builder{*node->declaration};
    node->typeinfo->declare(builder);
  }
  else {
    /* Declaration should have been created in #nodeRegisterType. */
    lib_assert(node->typeinfo->fixed_declaration != nullptr);
    node->declaration = node->typeinfo->fixed_declaration;
  }
  return true;
}

bool nodeDeclarationEnsure(NodeTree *ntree, Node *node)
{
 ...
 * ref other pointers which need validation. */
struct NodeClipboardExtraInfo {
  struct NodeClipboardExtraInfo *next, *prev;
  Id *id;
  char id_name[MAX_ID_NAME];
  char lib_name[FILE_MAX];
};
...
static NodeClipboard node_clipboard = {{nullptr}};

void node_clipboard_init(const struct NodeTree *ntree)
{
  node_clipboard.type = ntree->type;
}

void node_clipboard_clear()
{
  LIST_FOREACH_MUTABLE (NodeLink *, link, &node_clipboard.links) {
    nodeRemLink(nullptr, link);
  }
  lib_list_clear(&node_clipboard.links);

  LIST_FOREACH_MUTABLE (Node *, node, &node_clipboard.nodes) {
    node_free_node(nullptr, node);
  }
  lib_list_clear(&node_clipboard.nodes);

#ifdef USE_NODE_CB_VALIDATE
  lib_freelistn(&node_clipboard.nodes_extra_info);
#endif
}

bool node_clipboard_validate()
{
  bool ok = true;

#ifdef USE_NODE_CB_VALIDATE
  NodeClipboardExtraInfo *node_info;
  Node *node;

  /* lists must be aligned */
  lib_assert(lib_list_count(&node_clipboard.nodes) ==
             lib_list_count(&node_clipboard.nodes_extra_info));

  for (node = (Node *)node_clipboard.nodes.first,
      node_info = (NodeClipboardExtraInfo *)node_clipboard.nodes_extra_info.first;
       node;
       node = (Node *)node->next, node_info = (NodeClipboardExtraInfo *)node_info->next) {
    ...
    if (node->id) {
      ...
      List *lb = which_libbase(G_MAIN, GS(node_info->id_name));
      lib_assert(lb != nullptr);

      if (lib_findindex(lb, node_info->id) == -1) {
        /* May assign null. */
        node->id = (Id *)lib_findstring(lb, node_info->id_name + 2, offsetof(ID, name) + 2);
...
void node_clipboard_add_node(Node *node)
{
#ifdef USE_NODE_CB_VALIDATE
  /* add extra info */
  NodeClipboardExtraInfo *node_info = (NodeClipboardExtraInfo *)mem_mallocn(
      sizeof(NodeClipboardExtraInfo), __func__);

  node_info->id = node->id;
  if (node->id) {
    lib_strncpy(node_info->id_name, node->id->name, sizeof(node_info->id_name));
    if (ID_IS_LINKED(node->id)) {
      lib_strncpy(
          node_info->library_name, node->id->lib->filepath_abs, sizeof(node_info->lib_name));
    }
    else {
  ...
  lib_addtail(&node_clipboard.nodes_extra_info, node_info);
  /* end extra info */
#endif /* USE_NODE_CB_VALIDATE */

  /* add node */
  lib_addtail(&node_clipboard.nodes, node);
}

void node_clipboard_add_link(NodeLink *link)
{
  lib_addtail(&node_clipboard.links, link);
}

const List *node_clipboard_get_nodes()
{
  return &node_clipboard.nodes;
}

const List *node_clipboard_get_links()
{
  return &node_clipboard.links;
}

int node_clipboard_get_type()
{
  return node_clipboard.type;
}

void node_clipboard_free()
{
  node_clipboard_validate();
  node_clipboard_clear();
}

/* Node Instance Hash */
const NodeInstanceKey NODE_INSTANCE_KEY_BASE = {5381};
const NodeInstanceKey NODE_INSTANCE_KEY_NONE = {0};

/* Generate a hash key from ntree and node names
 * Uses the djb2 algorithm with xor by Bernstein:
 * http://www.cse.yorku.ca/~oz/hash.html */
static NodeInstanceKey node_hash_int_str(NodeInstanceKey hash, const char *str)
{
  char c;
...
NodeInstanceKey node_instance_key(NodeInstanceKey parent_key,
                                  const NodeTree *ntree,
                                  const Node *node)
{
  NodeInstanceKey key = node_hash_int_str(parent_key, ntree->id.name + 2);
...
{
  return ((const NodeInstanceKey *)key)->value;
}

static bool node_instance_hash_key_cmp(const void *a, const void *b)
{
  unsigned int value_a = ((const NodeInstanceKey *)a)->value;
  unsigned int value_b = ((const NodeInstanceKey *)b)->value;

  return (value_a != value_b);
}

NodeInstanceHash *node_instance_hash_new(const char *info)
{
  NodeInstanceHash *hash = (NodeInstanceHash *)mem_mallocn(sizeof(NodeInstanceHash), info);
  hash->ghash = lib_ghash_new(
      node_instance_hash_key, node_instance_hash_key_cmp, "node instance hash ghash");
  return hash;
}

void node_instance_hash_free(NodeInstanceHash *hash, NodeInstanceValueFP valfreefp)
{
  lib_ghash_free(hash->ghash, nullptr, (GHashValFreeFP)valfreefp);
  mem_freen(hash);
}

void node_instance_hash_insert(NodeInstanceHash *hash, NodeInstanceKey key, void *value)
{
  NodeInstanceHashEntry *entry = (NodeInstanceHashEntry *)value;
  entry->key = key;
  entry->tag = 0;
  lib_ghash_insert(hash->ghash, &entry->key, value);
}

void *node_instance_hash_lookup(NodeInstanceHash *hash, NodeInstanceKey key)
{
  return lib_ghash_lookup(hash->ghash, &key);
}

int node_instance_hash_remove(NodeInstanceHash *hash,
                              NodeInstanceKey key,
                              NodeInstanceValueFP valfreefp)
{
  return lib_ghash_remove(hash->ghash, &key, nullptr, (GHashValFreeFP)valfreefp);
}

void dune_node_instance_hash_clear(NodeInstanceHash *hash, NodeInstanceValueFP valfreefp)
{
  lib_ghash_clear(hash->ghash, nullptr, (GHashValFreeFP)valfreefp);
}

void *node_instance_hash_pop(NodeInstanceHash *hash, NodeInstanceKey key)
{
  return lib_ghash_popkey(hash->ghash, &key, nullptr);
}

int node_instance_hash_haskey(NodeInstanceHash *hash, NodeInstanceKey key)
{
  return lib_ghash_haskey(hash->ghash, &key);
}

int node_instance_hash_size(NodeInstanceHash *hash)
{
  return lib_ghash_len(hash->ghash);
}

void node_instance_hash_clear_tags(NodeInstanceHash *hash)
{
  NodeInstanceHashIterator iter;

  NODE_INSTANCE_HASH_ITER (iter, hash) {
    NodeInstanceHashEntry *value = (NodeInstanceHashEntry *)
...
void node_instance_hash_tag(NodeInstanceHash *UNUSED(hash), void *value)
{
  NodeInstanceHashEntry *entry = (NodeInstanceHashEntry *)value;
  entry->tag = 1;
}

bool node_instance_hash_tag_key(NodeInstanceHash *hash, NodeInstanceKey key)
{
  NodeInstanceHashEntry *entry = (NodeInstanceHashEntry *)node_instance_hash_lookup(hash, key);
...
void node_instance_hash_remove_untagged(NodeInstanceHash *hash,
                                        NodeInstanceValueFP valfreefp)
{
  /* NOTE: Hash must not be mutated during iterating!
   * Store tagged entries in a separate list and remove items afterward */
  NodeInstanceKey *untagged = (NodeInstanceKey *)mem_mallocn(
      sizeof(NodeInstanceKey) * node_instance_hash_size(hash),
      "temp node instance key list");
  NodeInstanceHashIter iter;
  int num_untagged = 0;
  NODE_INSTANCE_HASH_ITER (iter, hash) {
    NodeInstanceHashEntry *value = (NodeInstanceHashEntry *)
        node_instance_hash_iter_get_value(&iter);

    if (!value->tag) {
      untagged[num_untagged++] = node_instance_hash_iter_get_key(&iter);
    }
  }
...
static int node_get_deplist_recurs(NodeTree *ntree, Node *node, Node ***nsort)
{
  int level = 0xFFF;
...
void ntreeGetDependencyList(struct NodeTree *ntree, struct Node ***r_deplist, int *r_deplist_len)
{
  *r_deplist_len = 0;
  ...
  Node **nsort;
  nsort = *r_deplist = (Node **)mem_callocn((*r_deplist_len) * sizeof(bNode *),
                                             "sorted node array");
...
void ntreeUpdateNodeLevels(NodeTree *ntree)
{
  /* first clear tag */
  LIST_FOREACH (Node *, node, &ntree->nodes) {
    node->done = false;
...
void ntreeUpdateAllUsers(Main *main, Id *id)
{
  ...
  FOREACH_NODETREE_END;
  if (need_update) {
    ntree_update_main(main, nullptr);
  }
}

/* node type access */
void nodeLabel(const NodeTree *ntree, const Node *node, char *label, int maxlen)
{
  label[0] = '\0';

  if (node->label[0] != '\0') {
    lib_strncpy(label, node->label, maxlen);
  }
  else if (node->typeinfo->labelfn) {
    node->typeinfo->labelfn(ntree, node, label, maxlen);
  }
  ...
  if (label[0] == '\0') {
    ...
    const char *tmp = CXT_IFACE_(LANG_CXT_ID_NODETREE, node->typeinfo->ui_name);
    if (tmp == node->typeinfo->ui_name) {
      tmp = IFACE_(node->typeinfo->ui_name);
    }
    lib_strncpy(label, tmp, maxlen);
  }
}

const char *nodeSocketLabel(const NodeSocket *sock)
{
  return (sock->label[0] != '\0') ? sock->label : sock->name;
}

static void node_type_base_defaults(NodeType *ntype)
{
  /* default size values */
...
static bool node_poll_default(NodeType *UNUSED(ntype),
                              NodeTree *UNUSED(ntree),
                              const char **UNUSED(disabled_hint))
{
  return true;
}

/* use the basic poll fn */
static bool node_poll_instance_default(Node *node, NodeTree *ntree, const char **disabled_hint)
{
  return node->typeinfo->poll(node->typeinfo, ntree, disabled_hint);
}

void node_type_base(NodeType *ntype, int type, const char *name, short nclass)
{
  /* Use static type info header to map static int type to identifier string and RNA struct type.
   * Associate the api struct type with the NodeType.
   * Dynamically registered nodes will create an api type at runtime
   * and call api_struct_dune_type_set, so this only needs to be done for old RNA types
   * created in makesapi, which can not be associated to a NodeType immediately,
   * since NodeTypes are registered afterward .. */
#define DefNode(Category, Id, DefFn, EnumName, StructName, UIName, UIDesc) \
  case ID: \
    lib_strncpy(ntype->idname, #Category #StructName, sizeof(ntype->idname)); \
    ntype->rna_ext.srna = api_struct_find(#Category #StructName); \
...
#include "NOD_static_types.h"
  ...
  lib_assert(ntype->idname[0] != '\0');

  ntype->type = type;
  lib_strncpy(ntype->ui_name, name, sizeof(ntype->ui_name));
  ntype->nclass = nclass;
...
void node_type_base_custom(NodeType *ntype, const char *idname, const char *name, short nclass)
{
  lib_strncpy(ntype->idname, idname, sizeof(ntype->idname));
...
struct SocketTemplateIdCbData {
  NodeSocketTemplate *list;
  NodeSocketTemplate *ntemp;
};

static bool unique_socket_template_id_check(void *arg, const char *name)
{
  SocketTemplateIdCbData *data = (SocketTemplateIdCbData *)arg;

  for (NodeSocketTemplate *ntemp = data->list; ntemp->type >= 0; ntemp++) {
    if (ntemp != data->ntemp) {
      if (STREQ(ntemp->id, name)) {
        return true;
      }
    }
  }
...
static void unique_socket_template_id(NodeSocketTemplate *list,
                                      NodeSocketTemplate *ntemp,
                                      const char defname[],
                                      char delim)
{
  SocketTemplateIdentifierCbData data;
  data.list = list;
  data.ntemp = ntemp;

  lib_uniquename_cb(unique_socket_template_id_check,
                    &data,
                    defname,
                    delim,
                    ntemp->id,
...
void node_type_socket_templates(struct NodeType *ntype,
                                struct NodeSocketTemplate *inputs,
                                struct NodeSocketTemplate *outputs)
{
  ntype->inputs = inputs;
  ...
  if (inputs) {
    /* clear id strings (uninitialized memory) */
    for (NodeSocketTemplate *ntemp = inputs; ntemp->type >= 0; ntemp++) {
      ntemp->id[0] = '\0';
    }

    for (NodeSocketTemplate *ntemp = inputs; ntemp->type >= 0; ntemp++) {
      lib_strncpy(ntemp->id, ntemp->name, sizeof(ntemp->id));
      unique_socket_template_id(inputs, ntemp, ntemp->id, '_');
    }
  }
  if (outputs) {
    /* clear id strings (uninitialized memory) */
    for (NodeSocketTemplate *ntemp = outputs; ntemp->type >= 0; ntemp++) {
      ntemp->id[0] = '\0';
    }

    for (NodeSocketTemplate *ntemp = outputs; ntemp->type >= 0; ntemp++) {
      lib_strncpy(ntemp->id, ntemp->name, sizeof(ntemp->id));
      unique_socket_template_id(outputs, ntemp, ntemp->id, '_');
    }
  }
}

void node_type_init(struct NodeType *ntype,
                    void (*initfn)(struct NodeTree *ntree, struct Node *node))
{
  ntype->initfn = initfn;
}

void node_type_size(struct NodeType *ntype, int width, int minwidth, int maxwidth)
{
  ntype->width = width;
...
void node_type_size_preset(struct NodeType *ntype, eNodeSizePreset size)
{
  switch (size) {
...
void node_type_storage(NodeType *ntype,
                       const char *storagename,
                       void (*freefn)(struct Node *node),
                       void (*copyfn)(struct NodeTree *dest_ntree,
                                        struct Node *dest_node,
                                        const struct Node *src_node))
{
  if (storagename) {
    lib_strncpy(ntype->storagename, storagename, sizeof(ntype->storagename));
  }
  else {
...
void node_type_update(struct NodeType *ntype,
                      void (*updatefn)(struct NodeTree *ntree, struct Node *node))
{
  ntype->updatefn = updatefn;
}

void node_type_group_update(struct NodeType *ntype,
                            void (*group_update_fun)(struct NodeTree *ntree, struct Node *node))
{
  ntype->group_update_fn = group_update_fn;
}

void node_type_ex(struct NodeType *ntype,
                    NodeInitExecFunction init_ex_fn,
                    NodeFreeExFn free_ex_fn,
                    NodeExFn ex_fn)
{
  ntype->init_ex_fn = init_ex_fn;
  ntype->free_ex_fn = free_ex_fn;
  ntype->exec_fn = exec_fn;
}

void node_type_gpu(struct NodeType *ntype, NodeGPUExFn gpu_fn)
{
  ntype->gpu_fn = gpu_fn;
}

/* cbs for undefined types */
static bool node_undefined_poll(NodeType *UNUSED(ntype),
                                NodeTree *UNUSED(nodetree),
...
static void register_undefined_types()
{
  ...
  lib_strncpy(NodeSocketTypeUndefined.idname,
              "NodeSocketUndefined",
...
void dune_node_system_init()
{
  nodetreetypes_hash = lib_ghash_str_new("nodetreetypes_hash gh");
  nodetypes_hash = lib_ghash_str_new("nodetypes_hash gh");
  nodesockettypes_hash = lib_ghash_str_new("nodesockettypes_hash gh");
...
void BKE_node_system_exit()
{
  if (nodetypes_hash) {
    NODE_TYPES_BEGIN (nt) {
      if (nt->api_ext.free) {
        nt->api_ext.free(nt->api_ext.data);
      }
    }
    NODE_TYPES_END;

    lib_ghash_free(nodetypes_hash, nullptr, node_free_type);
  ...
  if (nodesockettypes_hash) {
    ...
    lib_ghash_free(nodesockettypes_hash, nullptr, node_free_socket_type);
  ...
  if (nodetreetypes_hash) {
    NODE_TREE_TYPES_BEGIN (nt) {
      if (nt->api_ext.free) {
        nt->api_ext.free(nt->rna_ext.data);
      }
    }
    NODE_TREE_TYPES_END;

    lib_ghash_free(nodetreetypes_hash, nullptr, ntree_free_type);
    nodetreetypes_hash = nullptr;
  }
}

/* NodeTree Iter Helpers (FOREACH_NODETREE_BEGIN) */
void dune_node_tree_iter_init(struct NodeTreeIterStore *ntreeiter, struct Main *main)
{
  ntreeiter->ngroup = (NodeTree *)main->nodetrees.first;
  ntreeiter->scene = (Scene *)main->scenes.first;
  ntreeiter->mat = (Material *)main->materials.first;
  ntreeiter->tex = (Tex *)main->textures.first;
  ntreeiter->light = (Light *)main->lights.first;
  ntreeiter->world = (World *)main->worlds.first;
  ntreeiter->linestyle = (FreestyleLineStyle *)main->linestyles.first;
  ntreeiter->simulation = (Simulation *)main->simulations.first;
}
bool dune_node_tree_iter_step(struct NodeTreeIterStore *ntreeiter,
                             NodeTree **r_nodetree,
                             struct Id **r_id)
{
  if (ntreeiter->ngroup) {
    *r_nodetree = (NodeTree *)ntreeiter->ngroup;
    *r_id = (Id *)ntreeiter->ngroup;
    ntreeiter->ngroup = (NodeTree *)ntreeiter->ngroup->id.next;
  }
  else if (ntreeiter->scene) {
    *r_nodetree = (NodeTree *)ntreeiter->scene->nodetree;
    *r_id = (Id *)ntreeiter->scene;
    ntreeiter->scene = (Scene *)ntreeiter->scene->id.next;
  }
  else if (ntreeiter->mat) {
    *r_nodetree = (NodeTree *)ntreeiter->mat->nodetree;
    *r_id = (Id *)ntreeiter->mat;
    ntreeiter->mat = (Material *)ntreeiter->mat->id.next;
  }
  else if (ntreeiter->tex) {
    *r_nodetree = (NodeTree *)ntreeiter->tex->nodetree;
    *r_id = (Id *)ntreeiter->tex;
    ntreeiter->tex = (Tex *)ntreeiter->tex->id.next;
  }
  else if (ntreeiter->light) {
    *r_nodetree = (NodeTree *)ntreeiter->light->nodetree;
    *r_id = (Id *)ntreeiter->light;
    ntreeiter->light = (Light *)ntreeiter->light->id.next;
  }
  else if (ntreeiter->world) {
    *r_nodetree = (NodeTree *)ntreeiter->world->nodetree;
    *r_id = (Id *)ntreeiter->world;
    ntreeiter->world = (World *)ntreeiter->world->id.next;
  }
  else if (ntreeiter->linestyle) {
    *r_nodetree = (NodeTree *)ntreeiter->linestyle->nodetree;
    ...
    *r_nodetree = (NodeTree *)ntreeiter->simulation->nodetree;
...
/* NodeTree kernel fns */
void nodetree_remove_layer_n(NodeTree *ntree, Scene *scene, const int layer_index)
{
  lib_assert(layer_index != -1);
  LIST_FOREACH (Node *, node, &ntree->nodes) {
