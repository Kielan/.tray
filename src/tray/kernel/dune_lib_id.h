...
struct Id *dune_libblock_find_session_uuid(struct Main *main, short type, uint32_t session_uuid);
...
ENUM_OPS(eLibIdFlagsDup, LIB_ID_DUPLICATE_IS_ROOT_ID)
 ...
 *
 * note low-level helper to de-duplicate logic between `dune_lib_id_make_local_generic` and the
...
void dune_lib_id_make_local_generic(struct Main *bmain, struct Id *id, int flags);
 ...
 *
 * param flags: Same set of `LIB_ID_MAKELOCAL_` flags as passed to dune_lib_id_make_local. */
void dune_lib_id_clear_library_data(struct Main *bmain, struct ID *id, int flags);

/* Clear or set given tags for all ids of given type in `bmain` (runtime tags).
 *
 * note Affect whole Main database. */
void dune_main_id_tag_idcode(struct Main *mainvar, short type, int tag, bool value);
/* Clear or set given tags for all ids in listbase (runtime tags). */
void dune_main_id_tag_list(struct List *lb, int tag, bool value);
/* Clear or set given tags for all ids in main (runtime tags). */
void dune_main_id_tag_all(struct Main *mainvar, int tag, bool value);

/* Clear or set given flags for all ids in listbase (persistent flags). */
void dube_main_id_flag_list(struct List *lb, int flag, bool value);
/* Clear or set given flags for all ids in main (persistent flags). */
void dune_main_id_flag_all(struct Main *main, int flag, bool value);

/* Next to indirect usage in `readfile.c/writefile.c` also in `editobject.c`, `scene.c`. */
void dune_main_id_newptr_and_tag_clear(struct Main *bmain);

void dune_main_id_refcount_recompute(struct Main *bmain, bool do_linked_only);
